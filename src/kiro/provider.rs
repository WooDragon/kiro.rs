...
mod tests {
    ...
    /// （`credit_weight_by_kiro_id` 精确命中 4.4，`#99` 官方倍率）。两次上游
    /// 调用各计一次权重，预算耗尽后不退款，最终 `load` 应约等于 `2 * 4.4 = 8.8`。
    ///
    ...
    /// （§B 既有行为，非本 PR 引入），衰减量与被衰减的 load 值成正比，权重从
    /// 2.4 改为 4.4（`#99`）后衰减绝对值等比放大但仍是 1e-5 量级，与 8.8 相差
    /// 远小于 1e-2。这是"计量恰好两次、不退款"这条真实保证之外的另一个真实
    /// 效应（衰减精度，已由 N3 覆盖），不该被这条测试的容差意外卡住。容差取
    /// `1e-2`——远宽于合理时钟抖动下的衰减量级（1e-5 ~ 1e-4 级），但仍严格
    /// 窄于"少计一次"(0.0 差 8.8)或"多计一次"(4.4 差 4.4)等真实回归的量级，
    /// 足以让下面的反事实（删计量行）可靠变红。
    #[tokio::test]
    async fn test_p3_record_upstream_call_meters_exactly_once_per_call_no_refund() {
        ...
        assert!(
            (load - 8.8).abs() < 1e-2,
            "load 应约等于 2 次调用 × credit_weight(gpt-5.6-sol)=4.4 = 8.8（容差 1e-2 内，见上方 doc comment），实际: {load}"
        );
    }
