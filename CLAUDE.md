...
- GPT-5.6 系列（`gpt-5.6-sol`/`terra`/`luna`）纯 `models.toml` 配置支持，零代码改动；`tier=pro`、`context_window=272_000`（Kiro 上游规格，OpenAI 原生 1M **不适用**）；不配 `thinking_type`/`thinking_effort`——上游对 prompt 内的 thinking 静默忽略（不报 400）；但 `additionalModelRequestFields` 是 **closed schema**，塞 `output_config`/`thinking` 键会 400 `property ... is not defined in the schema`（#78 实测）——**「thinking 被静默忽略」不可推论「output_config 也被忽略」**，两者行为不同；luna thinking 未独立实测、`output_config` 400 三变体均已实测（见私有 docs/gpt56-model-support-blackbox.md）
- effort 透传只对配置 `thinking_type = "adaptive"` 的模型生效（opus-5/4.8/4.7/4.6 + sonnet-5/4.6，共 6 个），其余模型**必须不产出** `additionalModelRequestFields`——opus-4.5/sonnet-4.5/haiku-4.5 上游返回 400 `additionalModelRequestFields is not supported for this model`，GPT-5.6 三变体 closed schema 拒键，给它们发该字段会让**整个请求 400**（护栏用例 `test_non_adaptive_models_never_produce_additional_fields` 钉死）；effort 取值链 `客户端 output_config.effort ?? models.toml thinking_effort ?? "high"`，**客户端优先不可覆盖**——CC 的 `/effort`/`--effort`/`CLAUDE_CODE_EFFORT_LEVEL`/settings `effortLevel` 全落在该字段（CC 2.1.221 实测会发送，旧结论「CC 从不发 output_config」已失效）；`thinking:{type:"enabled"}` 结构化上送上游 400（枚举只认 `["adaptive","disabled"]`），故结构化 thinking 恒填 adaptive 是上游契约要求；原「注入 `<thinking_mode>` 文本前缀」路径经 #62 实测无效已于 #78 删除（细节见私有 docs/effort-passthrough-config-driven.md，#78）
